...
public class Shader
{
	...
	public void updateUniforms(Transform transform, Material material, RenderingEngine renderingEngine)
	{
		...
		Matrix4f MVPMatrix = renderingEngine.get_main_camera().get_perspective().mul(worldMatrix);
		...
		for(int i = 0; i < resource.getUniformNames().size(); i++)
		{
			...
			else if(uniformName.startsWith("R_"))
			{
				...
				if(uniformType.equals("vec3"))
					...
					setUniformDirectionalLight(uniformName, (DirectionalLight)renderingEngine.getActive_light());
				else if(uniformType.equals("PointLight"))
					setUniformPointLight(uniformName, (PointLight)renderingEngine.getActive_light());
				else if(uniformType.equals("SpotLight"))
					setUniformSpotLight(uniformName, (SpotLight)renderingEngine.getActive_light());
				else
			...
			else if(uniformName.startsWith("C_"))
			{
				if(uniformName.equals("C_eyePos"))
					setUniform(uniformName, renderingEngine.get_main_camera().get_transform().getTransformedPos());
				else
	...
	private static String loadShader(String fileName)
	{
		...
		try
		{
			shaderReader = new BufferedReader(new FileReader("src/shaders/" + fileName));
			String line;
	...
	{
		glUniformMatrix4(resource.getUniforms().get(uniformName), true, Util.create_float_buffer(value));
	}
	...
	public void setUniformPointLight(String uniformName, PointLight pointLight)
	{
		...
		setUniform(uniformName + ".position", pointLight.get_transform().getTransformedPos());
		setUniformf(uniformName + ".range", pointLight.get_range());
	}
